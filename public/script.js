...
var peer = new Peer(undefined, {
  //we use undefined be cause the id is genereated by peer itself
...
const connectToNewUser = (userId, stream) => {
  //stream is coming from the promise(myvideostream)
  ...
  call.on("stream", (uservideoStream) => {
    //sending our video streaam
    addVideoStream(video, uservideoStream);
  });
  call.on("close", () => {
    //it removes the video of the user who leaves the meet
    video.remove();
  });
  peers[userId] = call;
};
...
const addVideoStream = (video, stream) => {
  //video-taking video object on the element
...
let text = $("input");
$("html").keydown((e) => {
  //$('html')it calls the html that is room.ejs
